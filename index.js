...
// Owner numbers
const ownerNumber = ["923376539373"];
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("SYED-MD~")) {
    console.log("❌ Invalid SYED-MD session format");
    process.exit(1);
  }
  ...
  console.log("♻️ SYED-MD session restored successfully");
}
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'SYED ABDUL WAHAB BUKHARI'}</p>
        </div>
      </body>
      </html>
    `);
...
async function connectToWA() {
  ...
  console.log(`🤖 SYED-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ SYED-MD Bot connected to WhatsApp!");
      ...
      try {
        ...
        sock.sendMessage(ownerNumber[0] + '923049730127@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
